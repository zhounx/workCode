<template>
  <div class="form-submit-page">
    <div class="message-top">
      <h2>提交开户资料</h2>
      <home-step :stepActive="2" :step-list="stepList.hongkongStep"></home-step>
        ...
        >
          ...
          <template #cardImage>
            <yk-upload
              ...
              settleInfoFormRule?.validateField('cardImage')
               }"
            >
            </yk-upload>
            <yk-upload
...
const reactiveLegalInfo = reactive(legalInfo);
...
watch(legalInfo.value, (newVal, oldValue) => {
  ...
  validateLegDate();
  hongkongApply.setPrivateSettle(legalInfo.value);
...
const defaultInfo =
  applyInfo.value?.settleInfo?.settleType === 1
    ? { ...hongkongApply.privateSettleInfo }
    : { ...hongkongApply.publicSettleInfo };
...
let settleInfoFormItems = ref<any[]>(
  applyInfo.value?.settleInfo?.settleType === 1
    ? settleInfoFormItems1
    : settleInfoFormItems2
);
...
const changeSettleType = (index: number) => {
  ...
  if (index === 1) {
    settleInfo.value.accountMobile =
      hongkongApply.privateSettleInfo.accountMobile;
    settleInfo.value.cardholder = hongkongApply.privateSettleInfo.cardholder;

    settleInfo.value.settleType = 1;
    settleInfoFormItems.value = settleInfoFormItems1;
  } else {
    settleInfo.value.cardholder = hongkongApply.publicSettleInfo.cardholder;
...
function goBack() {
  ElMessageBox.confirm("当前填写的内容未保存，确定返回上一步吗?", "温馨提示", {
    ...
    router.push({ path: "hongkongFirst" });
  });
}
...
async function twoSubmit() {
  let res: any = await hongkongApply.applyInfoAtSecondStep(
  ...
  if (res === 200) {
    ...
    router.push({ path: "hongkongThird" });
  } else {
    ElMessage({
      type: "warning",
      message: "网络繁忙, 请重试!",
