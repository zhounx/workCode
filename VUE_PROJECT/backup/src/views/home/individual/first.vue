        ...
        >
          <template #imageUrl>
            <yk-upload
              ...
              @on-success="(event:any)=>{licenseInfo.imageUrl=event.data;
              licenseInfoFormRule?.validateField('imageUrl')
              }"
              @submit="getOCRResult"
            ></yk-upload>
          </template>
          <template #legalHolderImage>
            <yk-upload
              ...
              @on-success="(event:any)=>{licenseInfo.legalHolderImage=event.data;
              licenseInfoFormRule?.validateField('legalHolderImage')
              }"
            ></yk-upload>
          </template>
  ...
  if (data.success) {
    switch (data.valueName) {
      case "licenseInfo.imageUrl": {
        ...
        individualApply.licenseOCR(data);
...
const goSecondStep = async () => {
  ...
  if (licenseInfoValidate && baseInfoValidate) {
    ...
    if (checkRes?.code === 200) {
      let res: any = await individualApply.applyInfoAtFirstStep(
      ...
      if (res === 200) {
        await baseInfoStore.getMerchantApplyInfo();
        individualApply.setSettleInfo(baseInfoStore.applyInfo);
        router.push({ path: `individualSecond` });
      } else {
        ElMessage({
          type: "warning",
          message: "网络繁忙, 请重试!",
