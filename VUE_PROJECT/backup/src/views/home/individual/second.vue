...
const individualApply = individualApplyStore();
...
if (applyInfo.value?.legalInfo?.expiryDate === "") {
...
watch(legalInfo.value, (newVal, oldValue) => {
  ...
  individualApply.changeSettleInfo(legalInfo.value);
...
const defaultInfo =
  applyInfo.value?.settleInfo?.settleType === 1
    ? { ...individualApply.privateSettleInfo }
    : { ...individualApply.publicSettleInfo };
...
const changeSettleType = (index: number) => {
  ...
  if (index === 1) {
    settleInfo.value.accountMobile =
      individualApply.privateSettleInfo.accountMobile;
    settleInfo.value.cardholder = individualApply.privateSettleInfo.cardholder;

    settleInfo.value.settleType = 1;
    settleInfoFormItems.value = settleInfoFormItems1;
  } else {
    settleInfo.value.cardholder = individualApply.publicSettleInfo.cardholder;
...
async function twoSubmit() {
  let res: any = await individualApply.applyInfoAtSecondStep(
  ...
  if (res === 200) {
    ...
    router.push({ path: "individualThird" });
  } else {
    ElMessage({
      type: "warning",
      message: "网络繁忙, 请重试!",
...
function goBack() {
  ElMessageBox.confirm("当前填写的内容未保存，确定返回上一步吗?", "温馨提示", {
    ...
    router.push({ path: "individualFirst" });
  });
}
//#endregion
</script>
